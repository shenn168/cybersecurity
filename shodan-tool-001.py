...
Press Enter to continue...")
    ...
    def display_menu(self):
        ...
        print("3.  Search Exploits - Search for exploits")
...
[ERROR] Unexpected error: {e}")
    ...
    def search_exploits(self):
        """Option 3: Search for Exploits"""
        self.clear_screen()
        print("=" * 60)
        print("  SEARCH EXPLOITS")
        print("=" * 60)
        print()
        
        query = input("Enter exploit search query (e.g., 'CVE-2021-44228'): ").strip()
...
[ERROR] Query cannot be empty!")
        ...
        try:
            print(f"\
[INFO] Searching exploits for: {query}")
            exploits = self.api.exploits.search(query)
            
            print(f"\
{'=' * 60}")
            print(f"Total exploits found: {exploits.get('total', 0)}")
            print(f"{'=' * 60}")
            
            if exploits.get('matches'):
                for i, exploit in enumerate(exploits['matches'], 1):
                    print(f"\
--- Exploit {i} ---")
                    print(f"CVE: {exploit.get('cve', ['N/A'])[0] if exploit.get('cve') else 'N/A'}")
                    print(f"Description: {exploit.get('description', 'N/A')[:200]}...")
